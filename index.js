...
class Player {
  ...
  smash() {
    ...
    for (var i = room.entities.length - 1; i >= 0; i--) {
      ...
      if (dist > (e.hitdist+this.axelength)**2) continue;
...
class Entity {
  ...
  moveForward(dir,speed) {
    ...
    if (room.checkCollisions(dir.x+this.x,this.y,this.bbox,this)) {
      dir.x = 0;
      /*if (Math.abs(dir.y) > 0.02 * speed)*/ dir.y = Math.sign(dir.y) * speed / 1.41421356237;
    }
    if (room.checkCollisions(this.x,dir.y+this.y,this.bbox,this)) {
      dir.y = 0;
      /*if (Math.abs(dir.y) > 0.02 * speed)*/ dir.x = Math.sign(dir.x) * speed / 1.41421356237;
    }
    this.x += dir.x;
    this.y += dir.y;
  }
