...
const ONPRESS   = 0;  // Triggers when pressed down
const ONRELEASE = 1;  // Triggers when held down
const ONHOLD    = 2;  // Triggers when released
...
class GuiContext {
  constructor() {
    ...
    // this.style.StyleColorsClassic(); // Uncomment for classic color theme
  }
 ...
 */
class GuiObject {
  constructor(label, x, y, w, h, callback = null) {
    this._active    = false;
    this._pactive   = false;
    this._hover     = false;
    ...
    this.mode       = ONPRESS; // interaction mode
    this.val        = 0;
    this.input      = true;     // receives input, false if for display only
    this.visible    = true;     // visibility flag for object; 
                                //  when not visible, input is overridden to false
    
    if (callback != null && typeof callback === "function") {
      this.callback = callback; // optional callback function can be passed in
    }
    else {
      this.callback = null;
    }
  ...
  update() {
    ...
    if (this.input && this.visible) {
      ...
      // Handle improper mode input from user.
      if (this.mode != ONPRESS && 
          this.mode != ONHOLD && 
          this.mode != ONRELEASE) {
        console.error("Interaction mode for " + this.label + " must be set to ONPRESS, ONHOLD, or ONRELEASE. Defaulting to ONPRESS.");
        this.mode = ONPRESS;
      }
      ...
      if (touches.length == 0 && mouseIsPressed) {
        ...
        if (this.checkHit(mouseX, mouseY)) {
          ...
          if ((this.mode == ONPRESS || this.mode == ONHOLD) && !this._pactive) {
            this.triggered = true;
          }
          else if (this.mode == ONHOLD) {
            this.triggered = true;
          }
          else {
            this.triggered = false;
        ...
        // If no mouse press or touch, account for triggered on release
        if (this.mode == ONRELEASE && !this.triggered && this._pactive) {
          this.triggered = true;
        }
        else {
          this.triggered = false;
...
class GuiToggle extends GuiObject {
  ...
  update() {
    ...
    // Handle improper mode input from user.
      if (this.mode == ONHOLD) {
        console.error("Interaction mode for GuiCheckbox objects must be set to ONPRESS or ONRELEASE. Defaulting to ONPRESS.");
        this.mode = ONPRESS;
      }
